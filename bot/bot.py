...
# Добавляем обработчик сообщений в диспетчер
...
def add_handlers(application: Application):
    ...
    #application.add_handler(
    #    CommandHandler("change", commands.Change(reply_to), filters=filters.users_or_chats)
    #)
...
def with_message_limit(func):
    ...
    async def wrapper(message: Message, context: CallbackContext, question: str) -> None:
        ...
        #db.add_new_user(user_id=message.from_user.id,chat_id=message.chat_id,username=message.from_user.name)
...
#    db.add_new_user(user_id=user_id,chat_id=message.chat_id, username=message.from_user.username )
    return answer
